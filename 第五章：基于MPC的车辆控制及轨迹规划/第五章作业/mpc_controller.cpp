...
void FG_eval::operator()(ADvector &fg, ADvector &vars)
{
    ...
    {
        CppAD::AD<double> e_cte = vars[cte_start+t]-ref_cte;
        CppAD::AD<double> e_epsi = vars[epsi_start+t]-ref_epsi;
        CppAD::AD<double> e_v_longitudinal = vars[v_longitudinal_start+t]-ref_v;
        fg[0] += cte_weight * pow(e_cte,2); 
        fg[0] += epsi_weight * pow(e_epsi,2);
        fg[0] += v_weight * pow(e_v_longitudinal,2); 
    }
    ...
    {
        CppAD::AD<double>change_steer = vars[front_wheel_angle_start+t+1]-vars[front_wheel_angle_start+t];
        CppAD::AD<double>change_accel = vars[longitudinal_acceleration_start+t+1]-vars[longitudinal_acceleration_start+t];
        fg[0] += change_steer_cost_weight * pow(change_steer,2);
        fg[0] += change_accel_cost_weight * pow(change_accel,2);
    }
    ...
    for (size_t t = 1; t < Np; t++) // 预测模型
    {
        ...
        /* 全局坐标系 */ 
        CppAD::AD<double> vx_in_world_frame =v_longitudinal_0*cos(psi_0)+v_lateral_0*(-sin(psi_0));
        CppAD::AD<double> vy_in_world_frame =v_longitudinal_0*sin(psi_0)+v_lateral_0*(cos(psi_0));
        fg[1 + x_start + t] = x_1 - (x_0 + vx_in_world_frame*dt);
        fg[1 + y_start + t] = y_1 - (y_0 + vy_in_world_frame*dt);
        ...
        /* 车辆侧向速度 */
        CppAD::AD<double> delta = -front_wheel_angle_0;
        CppAD::AD<double> theta_vf = atan((v_lateral_0+lf*yaw_rate_0)/v_longitudinal_0);
        CppAD::AD<double> F_yf = 2* Cf * (delta - theta_vf);
        CppAD::AD<double> theta_vr = atan((v_lateral_0-lr*yaw_rate_0)/v_longitudinal_0);
        CppAD::AD<double> F_yr = 2*Cr*(-theta_vr);
        CppAD::AD<double> lateral_acceleration = (1/m)*(F_yr+F_yf*cos(delta)-m*v_longitudinal_0*yaw_rate_0);
        fg[1 + v_lateral_start + t] = v_lateral_1-(v_lateral_0+lateral_acceleration*dt);
        
        /* 车辆横摆角速度 */
        CppAD::AD<double> yaw_rate_rate = (1/I)*(F_yf*lf*cos(delta) - F_yr * lr);
        fg[1 + yaw_rate_start + t] = yaw_rate_1 - (yaw_rate_0 + yaw_rate_rate*dt) ;
